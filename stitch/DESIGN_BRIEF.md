...
- Approval Loop Probe turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page utility called Approval Loop Probe. It should show three compact status tiles, one Refresh button that updates a visible timestamp, and one Ready/Paused toggle. Keep it local-only, no database, no auth, no external APIs, and suitable for a quick smoke test.
...
- Record Operations - Approval Loop Probe
- Record Editor - Approval Loop Probe
- Insights - Approval Loop Probe

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Record Operations - Approval Loop Probe
...
SCREEN_SPEC_2:
- exact_screen_title: Record Editor - Approval Loop Probe
...
SCREEN_SPEC_3:
- exact_screen_title: Insights - Approval Loop Probe
